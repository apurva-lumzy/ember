...
class Media {
  ...
  createShader() {
    ...
    img.src = typeof this.image === 'string' ? this.image : (this.image?.image || '');
    img.onload = () => {
      texture.image = img;
  ...
  update(scroll) {
    ...
    // Update corresponding HTML overlay position (if it exists)
    const screenHeight = this.screen.height;
    const Y_px = - (this.plane.position.y / viewportHeight) * screenHeight;

    const cardEl = document.getElementById(`flying-poster-card-${this.index}`);
    if (cardEl) {
      cardEl.style.transform = `translate(-50%, -50%) translateY(${Y_px}px)`;
      
      const screenY = Y_px + screenHeight / 2;
      const halfHeight = this.planeHeight / 2;
      if (screenY + halfHeight < 0 || screenY - halfHeight > screenHeight) {
...
class Canvas {
  ...
  onTouchMove(e) {
    if (this.isDestroyed || !this.isDown) return;
...
export default function FlyingPosters({
  ...
  return (
    <div ref={containerRef} className={`w-full h-full overflow-hidden relative z-2 ${className}`} {...props}>
      ...
      {/* HTML Overlays positioned on top of the floating WebGL planes */}
        ...
        {items.map((item, idx) => {
          ...
          return (
            ...
            >
              <div 
                style={{
                  background: 'linear-gradient(to top, rgba(10, 8, 6, 0.95) 0%, rgba(10, 8, 6, 0.6) 70%, rgba(10, 8, 6, 0) 100%)',
